...
import locale
from flask import Flask, request, jsonify  # Corrigir importação
...
def enviar_emails(data_inicial = None, data_final = None):
    ...
    clientes_data = fetch_processes_and_clients(data_inicial,data_final)
    contador_Inativos = 0
    ...
    for cliente, processos in clientes_data.items():
        ...
        for processo in processos:
            ...
            if cliente_number and isinstance(cliente_number, list):
                ...
                numero = f"Cliente não tem número cadastrado na API"

            # atualiza o processo quando enviado
            status_processo(processo_id)

            # insere o processo enviado em "envio_emails"
...
def api_enviar_emails():
    ...
    # Aqui você pode chamar sua função para enviar os e-mails
    status, code = enviar_emails(data_inicial, data_final)
...
if __name__ == "__main__":
    ...
    app.run(host='localhost', port=8080)  # Especifica o host corretamente
